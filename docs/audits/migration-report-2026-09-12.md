...
source-blob: dba5fe4ff1e13328d96de9c4b4d4421387880478  (BACKLOG.md)
...
| quantity | count |
|---|---|
| top-level bullets in `BACKLOG.md` | 11 |
| of those, ENTRIES (bold, or led by a grade-shaped word) | 11 |
| of those, non-entry prose bullets (not migrated) | 0 |
| of those, bullets in a section §4 row 1 CUTS | 0 |
| items written to `ITEMS.md` | 2 |
...
**Identity:** 11 entries read = 2 written + 9 closed + 0 unclassified — HOLDS. THREE columns, not two: a closure read out of the source carrier is neither written as an item nor unclassified, and folding it into either would make one of those numbers say something it does not.

**Bullet identity:** 11 top-level bullets = 11 entries + 0 prose + 0 cut — HOLDS. This is the identity that makes 'not migrated' visible: every bullet in the source is in exactly one of the three columns, so a bullet the migration simply did not see would show up as a gap in the sum rather than as nothing at all.

**Conservation (§3.1), computed on the produced files:** items 3 (of which 1 residue) + done 9 = 12; baseline 12 + added 0 − compacted 0 = 12. HOLDS. THE RESIDUE IS IN THIS IDENTITY AND NOT IN THE ONE ABOVE, and the split is the point: a residue item is a BODY in the carrier, so conservation must see it, and it comes from no SOURCE ENTRY, so the reconciliation identity must not — folding it in there would answer COULD NOT VERIFY on every migration after this one.
...
|---|---|---|
| `(ungraded)` | CLOSED → the done home | 9 |
| `READY` | READY | 1 |
| `PARKED` | NEW | 1 |
...
|---|---|---|---|
| `BACKLOG.md:30-54` | `(ungraded)` | Done | §4 row 1 + lc-18: ungraded, under the carrier's own closure heading — the heading is the closure statement, an |
| `BACKLOG.md:55-83` | `(ungraded)` | Done | §4 row 1 + lc-18: ungraded, under the carrier's own closure heading — the heading is the closure statement, an |
| `BACKLOG.md:84-108` | `(ungraded)` | Done | §4 row 1 + lc-18: ungraded, under the carrier's own closure heading — the heading is the closure statement, an |
| `BACKLOG.md:109-117` | `(ungraded)` | Done | §4 row 1 + lc-18: ungraded, under the carrier's own closure heading — the heading is the closure statement, an |
| `BACKLOG.md:118-120` | `(ungraded)` | Done | §4 row 1 + lc-18: ungraded, under the carrier's own closure heading — the heading is the closure statement, an |
| `BACKLOG.md:121-126` | `(ungraded)` | Done | §4 row 1 + lc-18: ungraded, under the carrier's own closure heading — the heading is the closure statement, an |
| `BACKLOG.md:127-131` | `(ungraded)` | Done | §4 row 1 + lc-18: ungraded, under the carrier's own closure heading — the heading is the closure statement, an |
| `BACKLOG.md:132-147` | `(ungraded)` | Done | §4 row 1 + lc-18: ungraded, under the carrier's own closure heading — the heading is the closure statement, an |
| `BACKLOG.md:148-154` | `(ungraded)` | Done | §4 row 1 + lc-18: ungraded, under the carrier's own closure heading — the heading is the closure statement, an |
...
**7 tracked file(s)** still name `BACKLOG.md` — one item, its `write-set` and `evidence` naming every one of them:

- `CLAUDE.md`
- `PLAN.md`
- `dev-notes/OBSERVATIONS.md`
- `dev-notes/eval-begehung/2026-08-11-opus/tier2-with.md`
- `dev-notes/eval-begehung/2026-08-11/tier2-sentence.md`
- `dev-notes/eval-begehung/2026-08-11/tier2-with.md`
- `dev-notes/eval-begehung/2026-08-11/tier2-without.md`
